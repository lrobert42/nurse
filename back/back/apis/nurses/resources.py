...
from .models import NurseModel, NurseSchema, AgendaSchema
...
class Nurses(Resource):
    ...
    @accepts(schema=NurseSchema(), api=ns)
...
class NurseAgenda(Resource):
    ...
    # @ns.doc(params={
    #         "date":
    #             'Query filter on date:\n'
    #             'Examples:\n'
    #             '   `.../agenda?date="2020-01-25`'
    #         }
    #         )
...
# @ns.route("/<int:rid>/agenda/<int:idx>")
# @ns.doc(params={"rid": "The nurse rid (index start from 1)"})
# @ns.doc(params={"idx": "The work calendar agenda index in the list (index start from 0)"})
# class NurseAgendaEntry(Resource):
#     """Handles individual nurse agenda entries."""

#     @staticmethod
#     def _check_idx(agenda, idx):
#         if idx >= len(agenda) or idx < 0:
#             abort(404, "Agenda entry not found.")

#     @responds(schema=AgendaDaySchema(), api=ns)
#     @manager_or_owner_required
#     def get(self, rid, idx):
#         """Return a single agenda entry."""
#         entries = NurseModel.with_rid(rid).agenda.entries
#         self._check_idx(entries, idx)
#         return entries[idx]

#     @accepts(schema=AgendaDaySchema(partial=True), api=ns)
#     @responds(schema=AgendaDaySchema(), api=ns)
#     @manager_or_owner_required
#     def put(self, rid, idx):
#         """Update an agenda entry."""
#         nurse = NurseModel.with_rid(rid)
#         self._check_idx(nurse.agenda.entries, idx)
#         modified = nurse.agenda.modify(idx, request.parsed_obj)
#         nurse.save()
#         return modified
