...
nurseSchema = NurseSchema(only=('rid', 'treatment_types', 'address', 'agenda'))
...
def remap_keys(data):
    ...
    for entry in data["nurses"]:
        tmp = dict()
        tmp["_id"] = entry["rid"]
        tmp["treatmentTypesIds"] = entry["treatment_types"]
...
def handle_response(appointments, map):
...
def request_algo(date, algo_url: str):
    ...
    formated = remap_keys(data)
    pprint(formated, indent=2)
    return
    r = requests.post("http://" + algo_url, json=data)
    print(r.json())
